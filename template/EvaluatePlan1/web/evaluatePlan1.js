...
doSearch = function () {
    ...
    var double1 = $('#right_longSearchCombobox').val();
    var url = "getallevaluateplan1.json";
    url += "?key=" + keySearch;
...
$(function () {
	doSearch();
		...
		vt_combobox.buildMultipleCombobox("cbplace1Search", "gettypeuser.json?cd=705", 0, "dvsName", "dvsValue", "- Chọn Địa điểm lập kế hoạch 1 -", 0);
		vt_combobox.buildMultipleCombobox("cbdept_idSearch", "getdepartmentComBoBox.json", 0, "deptName", "deptId", "- Chọn Tên tổ chức, đơn vị được thẩm định -", 0);
        ...
        if (id !== null) {
            ...
            $.ajax({
                ...
                success: function (data, status, xhr) {
					...
					$("#dept_idView").val(data.dept_idST);
					$("#dept_otherView").val(data.dept_other);
					$("#the_basesView").val(data.the_bases);
					$("#the_purposeView").val(data.the_purpose);
					$("#place_and_timeView").val(data.place_and_time);
					...
					$("#left_longView").val(data.left_long);
					$("#right_longView").val(data.right_long);
